...
function getPage(succeed, fail) {
  page ++;
  const url = 'https://cmr.earthdata.nasa.gov/search/collections.json?page_size=' + page_size + '&page_num=' + page+'&pretty=true';
  curl(['-s', url])
    .then(function (res) {
...
function close(msg) {
  ...
  ws.write(fileEnd);
  ws.end();
  spawn('python', ['-m json.tools', 'collections.min.json', '>', 'collections.json']);
}
